...
map1=np.zeros((endx+256-startx,endy+256-starty))
for ii in range(startx,endx+256,1):
    for jj in range(starty,endy+256,1):
        if scan[ (scan[:,0]==ii) & (scan[:,2]==jj) ].shape[0]!=0:
            map1[ii-startx][jj-starty]=64+np.max(scan[ (scan[:,0]==ii) & (scan[:,2]==jj) ][:,1])
            #if map1[ii-startx][jj-starty]!=0: print ("929",ii,jj)

print(map1)
map1[map1>256]=255
map1=map1.reshape(map1.shape[0],map1.shape[1],1)
res_model=model.predict(np.expand_dims(map1/256,axis=0))
im=res_model[0][:,:,0].reshape(map1.shape[0],map1.shape[1],1)
im*=255
...
save_img("final_map.png",im,scale=False)                 
