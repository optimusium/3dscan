...
#model = load_model('map_network6_try_classifier.hdf5')
#model.load_weights("map_network6_try_weight.hdf5")
model = load_model('map_network6_resnet_classifier.hdf5')
model.load_weights("map_network6_resnet_weight.hdf5")
...
for kk in range(0,10,1):
    ...
    for ii in range(0, map1.shape[0], slicesize):
        for jj in range(0, map1.shape[1], slicesize):
            ...
            im[ (im>220) & (map6>im) ]=map6[(im>220) & (map6>im)]
            im=im.astype('int')
